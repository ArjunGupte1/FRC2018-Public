...
public class PathfinderController extends SynchronousPID implements Controller {
  ...
  private double kv = 1.0 / Constants.Drive.DRIVE_MAX_VELOCITY;
  private double ka = 0.0;
  private double last_left_error = 0.0;
  private double last_right_error = 0.0;
  private double current_heading = 0.0;
  private double initial_robot_heading = Drive.getInstance().getGyroAngle();
  ...
  private double angleDifference = 0.0;
  ...
  public synchronized void run() {
    ...
    if (onTarget()) {
      ...
      if (current_segment < leftTraj.length()) {
        ...
        lError = 0.0;
        double rError = 0.0;
        double measuredPosL = (mDrive.getLeftDist() - initial_left_encoder_dist);
        double measuredPosR = (mDrive.getRightDist() - initial_right_encoder_dist);
        if (isBackwards){
          lError =
        ...
        if (Constants.DEBUG_MODE) {
          SmartDashboard.putNumber("Pathfinder_segment", current_segment);
          
          SmartDashboard.putNumber("Pathfinder_lPos", measuredPosL);
          SmartDashboard.putNumber("Pathfinder_rPos", measuredPosR);
          SmartDashboard.putNumber("Pathfinder_lError", lError);
          SmartDashboard.putNumber("Pathfinder_rError", rError);

          SmartDashboard.putNumber("Pathfinder_lDistInit", initial_left_encoder_dist);
          SmartDashboard.putNumber("Pathfinder_rDistInit", initial_right_encoder_dist);
          ...
          SmartDashboard.putNumber("Pathfinder_lEncPos", mDrive.getLeftDist());
          SmartDashboard.putNumber("Pathfinder_rEncPos", mDrive.getRightDist());
        }

        lVelocity = lSegment.velocity * Constants.UnitConv.MPS_TO_FPS;
        ...
        double gyro_heading = DaisyMath.boundAngleNeg180to180Degrees(mDrive.getGyroAngle() - initial_robot_heading); // Assuming the
        // value in degrees

        angleDifference = DaisyMath
        ...
        double thisVelocityL = mDrive.getLeftVelocity();
        double thisVelocityR = mDrive.getRightVelocity();
        ...
        turn = DaisyMath.minmax(.6 * (1.0 / 120.0) * angleDifference, -0.6, 0.6); //.8
        ...
        if (Constants.DEBUG_MODE) {
          ...
          SmartDashboard.putNumber("Pathfinder_lTraj", leftTraj.length());
          SmartDashboard.putNumber("Pathfinder_rTraj", rightTraj.length());
          ...
          SmartDashboard.putNumber("Gyro Yaw", gyro_heading);
        }
  ...
  public synchronized void reset() {
    ...
    last_left_error = 0;
    last_right_error = 0;
    current_heading = mDrive.getGyroAngle();
    initial_left_encoder_dist = mDrive.getLeftDist();
    initial_right_encoder_dist = mDrive.getRightDist();
    initial_robot_heading = mDrive.getGyroAngle();
